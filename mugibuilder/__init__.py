...
class Commander:
    ...
    def install_aqt(self):
        self.add_python_path()
        self.add_7z_path() # aqt uses 7z
        self._aqt = True
    ...
    def cmake_generate(self, compiler, arch, path, *opts):
        if compiler == MSVC_2019:
            arch_ = {
                ARCH_32: 'Win32',
                ARCH_64: 'x64'
            }
            cmd = 'cmake -G "Visual Studio 16 2019" -A {}'.format(arch_[arch])
        elif is_mingw(compiler):
            cmd = 'cmake -G "MinGW Makefiles"'
        else:
            raise ValueError("cmake_generate not implemented for {}".format(compiler))
        cmd = " ".join([cmd, *opts, path])
        self._cmds.append(cmd)
...
class Workflow:
    ...
    def release(self):
        self._steps_github.append(self._release.github())
    
    def local_step(self, step):
        self._steps_local.append(step)

    def github_step(self, step):
        self._steps_github.append(step)

    def add_to_release(self, artifact):
        self._release.add(artifact)
    
    def save(self, base, on=ON_TAG, runs_on=WINDOWS_2019):
        workflow_path = os.path.join(base, ".github", "workflows", "main.yml")
        save_workflow(workflow_path, self._steps_github, on=on, runs_on=runs_on)
        batch_path = os.path.join(base, "main.bat")
        save_batch(batch_path, self._steps_local)
...
class ArgumentParser:
    ...
    def parse_args(self):
        ...
        if self._qt:
            qt, compilers, archs = get_specs_qt(args)
            return qt, compilers, archs, args
        else:
            compilers, archs = get_specs(args)
            return compilers, archs, args
